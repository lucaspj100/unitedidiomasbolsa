...
const FLOW: Step[] = [
  ...
  {
    kind: "choice", field: "nivel_ingles",
    label: "Como você avalia seu nível atual de inglês?",
    options: ["Básico", "Intermediário", "Avançado", "Não sei avaliar"],
  },
  {
    kind: "choice", field: "motivo_ingles",
    label: "Por que o inglês é importante para você hoje?",
    options: ["Crescimento profissional", "Melhor oportunidade de emprego", "Viagem", "Vida acadêmica", "Objetivo pessoal", "Outro"],
  },
  {
    kind: "choice", field: "impacto_ingles",
  ...
  {
    kind: "choice", field: "perdeu_oportunidade",
  ...
  {
    kind: "choice", field: "motivo_nao_faz_curso",
    label: "Por que você ainda não está fazendo um curso de inglês?",
    options: ["Falta de tempo", "Valor alto", "Já tentei antes e parei", "Não encontrei uma metodologia boa", "Falta de disciplina", "Outro"],
  },
  {
    kind: "choice", field: "decisao_entrevista",
    label: "Caso seu perfil seja selecionado para a bolsa, você está decidido a conversar em uma entrevista online para entender metodologia, valores e condições?",
...
function ChatPage() {
  ...
  useEffect(() => {
    setMessages([
      { from: "bot", text: "Olá! Vi que você tem interesse no processo de bolsa da United Idiomas." },
  ...
  // Auto-scroll: keep last message visible above the controls
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cls === "quente" || cls === "morno") {
        ...
        setStepIndex((i) => i + 1); // go to schedule
      } else {
        await persist({}, { status: cls === "frio" ? "Perfil não aprovado" : "Não agendou", etapa: "encerrado", classificacao: cls, alta });
        setStepIndex(FLOW.length - 1); // jump to done
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stepIndex]);

  // Load slots when entering schedule step
  useEffect(() => {
    ...
    <div className="flex min-h-screen flex-col bg-background">
        ...
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs opacity-80 leading-tight">{branding.brand_subtitle}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col px-3 sm:px-4">
        <div ref={scrollRef} className="flex-1 flex-col gap-3 overflow-y-auto py-4" style={{ display: "flex" }}>
            ...
            <div className="shadow-card mt-3 rounded-2xl border border-border bg-card p-4">
              ...
              {loadingSlots ? (
                ...
                <div className="text-sm text-muted-foreground">
                  {noSlotsMessage
                    ? "Seu cadastro foi registrado. Em breve entraremos em contato pelo WhatsApp para combinar um novo horário."
          ...
          {/* Spacer so the last bubble never sits under the controls */}
            ...
            {step?.kind === "schedule" && (
              <p className="text-center text-xs text-muted-foreground">
                {bookedAt
                  ...
                  : slots.length === 0
                  ? "Aguardando liberação de novos horários."
                  : "Para concluir sua pré-aprovação, escolha um dos horários disponíveis."}
              </p>
            )}
          </div>
        )}
